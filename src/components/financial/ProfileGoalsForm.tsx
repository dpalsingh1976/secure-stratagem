      ...
      <div className="space-y-8">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Set risk thresholds and preferences for personalized recommendations.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-8">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>Maximum acceptable peak-to-trough decline in portfolio value</p>
                    </TooltipContent>
                  </Tooltip>
                </div>
                <Badge variant="outline">{data.drawdown_tolerance}%</Badge>
              </div>
              <Slider
                value={[data.drawdown_tolerance]}
                onValueChange={([value]) => handleInputChange('drawdown_tolerance', value)}
                max={50}
                min={5}
                step={5}
                className="w-full"
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <Label>Liquidity Buffer (Months of Expenses)</Label>
                  <Tooltip>
                    <TooltipTrigger>
                      <InfoIcon className="h-4 w-4 text-gray-400" />
                    </TooltipTrigger>
                    <TooltipContent>
                      <p>Preferred months of expenses to keep in liquid cash reserves</p>
                    </TooltipContent>
                  </Tooltip>
                </div>
                <Badge variant="outline">{data.liquidity_buffer_months} months</Badge>
              </div>
              <Slider
                value={[data.liquidity_buffer_months]}
                onValueChange={([value]) => handleInputChange('liquidity_buffer_months', value)}
                max={24}
                min={3}
                step={1}
                className="w-full"
              />
              <div className="flex justify-between text-sm text-gray-500">
                <span>Minimal (3 mo)</span>
                <span>Conservative (24 mo)</span>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <Label>Single Asset Concentration Limit</Label>
                  <Tooltip>
                    <TooltipTrigger>
                      <InfoIcon className="h-4 w-4 text-gray-400" />
                    </TooltipTrigger>
                    <TooltipContent>
                      <p>Maximum percentage of net worth in any single investment</p>
                    </TooltipContent>
                  </Tooltip>
                </div>
                <Badge variant="outline">{data.concentration_threshold}%</Badge>
              </div>
              <Slider
                value={[data.concentration_threshold]}
                onValueChange={([value]) => handleInputChange('concentration_threshold', value)}
                max={50}
                min={5}
                step={5}
                className="w-full"
              />
              <div className="flex justify-between text-sm text-gray-500">
                <span>Diversified (5%)</span>
                <span>Concentrated (50%)</span>
              </div>
            </div>
          </CardContent>
        </Card>
