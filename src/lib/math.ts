...
/**
 * Simple financial math helpers for IUL comparisons.
 */

export function irr(cashflows: number[], guess = 0.1): number | null {
  // Newton-Raphson IRR; cashflows[0] usually negative (premium)
...
export function stressReturn(base: number, deltaBps: number): number {
  return Math.max(0, base + deltaBps / 100); // floor at 0%
}
