...
const IUL_ILLUSTRATED_RATE = 0.06; // Conservative 6.0% illustrated rate
...
function simulateScenarioA(
  ...
  const guaranteedMonthly = projection.income_sources.social_security + projection.income_sources.pension;

  // Target withdrawal - use fallback if monthly_income_target is 0 or undefined
  const targetIncome =
    projection.monthly_income_target ||
  ...
  return {
    ...
    scenario_description:
      "Continue current strategy with 401(k)/IRA contributions. All retirement income comes from taxable withdrawals.",
...
function simulateScenarioB(
  ...
  if (
    ...
    iulAnnualPremium =
      allocationOverrides?.iul_annual_premium && allocationOverrides.iul_annual_premium > 0
  ...
  const meetsCriteria =
    prefersGuaranteed || (hasIncomeGap && nearRetirement) || sequenceRiskHigh || lowGuaranteedCoverage;
  ...
  const ageQualifiesForAnnuity =
    currentAge >= AGE_MINIMUM_FOR_ANNUITY || yearsToRetirement <= AGE_EXCEPTION_YEARS_TO_RETIREMENT;
  ...
  if (meetsCriteria && hasMinimumAssets && ageAllowsAnnuity) {
    ...
    annuityPremium =
      allocationOverrides?.annuity_one_time_premium !== undefined && allocationOverrides.annuity_one_time_premium > 0
  ...
  if (includesIUL && yearsToRetirement > 0) {
    ...
    const loanableValue = iulCashValueAtRetirement * 0.8;
    ...
    iulDeathBenefit =
      allocationOverrides?.iul_death_benefit && allocationOverrides.iul_death_benefit > 0
  ...
  const iulContributionReduction = includesIUL
  ...
  // Portfolio at retirement - IUL is treated as ADDITIONAL investment, not reduction
  // The IUL premium is a new allocation, not redirected from existing contributions
  ...
  const guaranteedMonthly =
    projection.income_sources.social_security + projection.income_sources.pension + annuityGuaranteedIncome;
  ...
  return {
    ...
    retirement_income_net: netMonthly,
    lifetime_taxes_paid: lifetimeTaxes,
    has_guaranteed_income:
      guaranteedMonthly > projection.income_sources.social_security + projection.income_sources.pension,
...
function generatePlainEnglishSummary(
  ...
  if (scenarioB.legacy_value_at_90 > scenarioA.legacy_value_at_90 * 1.2) {
    parts.push("Additionally, the optimized strategy may leave a larger legacy for your heirs.");
  }
...
export function computeScenarioComparison(
  ...
  const scenarioA = simulateScenarioA(profileData, incomeData, assets, preferences, projection);
  ...
  const { includesIUL, includesAnnuity, iulReason, annuityReason, annuityEligibility, iulEligibility, ...scenarioB } =
    scenarioBResult;

  // Calculate total allocation for Scenario C comparison
  const totalAllocation =
    (scenarioB.iul_annual_premium || 0) * getYearsToRetirement(profileData.dob, profileData.retirement_age || 65) +
  ...
  let comparisonVsAlternative:
    | { income_improvement_monthly: number; tax_savings_lifetime: number; legacy_improvement_amount: number }
    | undefined;
  ...
  const incomeImprovementPercent =
    scenarioA.retirement_income_net > 0 ? (incomeImprovementMonthly / scenarioA.retirement_income_net) * 100 : 0;
  ...
  return {
    ...
    disclaimer:
      "DISCLAIMER: These projections are for educational purposes only and are based on illustrated, " +
