-- Allianz 222+ FIA intake tables
-- SSN/TIN and government ID stored AES-256-GCM encrypted (edge function encrypts before insert)
...
CREATE TABLE IF NOT EXISTS public.annuity_applications (
  ...
  last_name                        text,
  suffix                           text,
  ssn_tin                          text,            -- encrypted: base64(12-byte IV || GCM ciphertext)
  date_of_birth                    date,
  gender                           text,
  ...
  id_document_number               text,            -- encrypted
...
CREATE TABLE IF NOT EXISTS public.application_beneficiaries (
  ...
  relationship     text,
  date_of_birth    date,
  ssn_tin          text,   -- encrypted if provided
...
-- Inserts go through the edge function (service_role key, bypasses RLS).
-- No anon INSERT policies are granted intentionally.
